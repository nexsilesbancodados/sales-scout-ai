...
export function WebSearchTab() {
  ...
  const handleDirectoriesSearch = async () => {
    ...
    try {
      // Search in Brazilian directories via Firecrawl
      const directories = [
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Encontre leads em diferentes fontes: Google, scraping de sites ou diretórios brasileiros
          </CardDescription>
        </CardHeader>
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedResults.size > 0 && (
                <Button onClick={saveSelectedAsLeads} disabled={isSaving}>
                  ...
                  Salvar {selectedResults.size} como Leads
                </Button>
              )}
            </div>
          </CardHeader>
              ...
              <div className="space-y-3">
                {searchSource === 'firecrawl' ? (
                  ...
                  results.map((result, idx) => (
                    <Card 
                      key={idx} 
                      className={`p-4 cursor-pointer transition-all ${selectedResults.has(idx) ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'}`}
                      onClick={() => toggleResultSelection(idx)}
                    >
                      <div className="flex items-start gap-3">
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${selectedResults.has(idx) ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground'}`}>
