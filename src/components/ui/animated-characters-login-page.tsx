...
export function AnimatedCharactersLogin({
  ...
  error: externalError,
  brandName = "Prospecte",
  brandTagline = "Automatize sua prospecção com IA",
        ...
        <div className="w-full max-w-[420px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" className="w-full h-12 text-base font-medium" size="lg" disabled={isLoading}>
              {isLoading ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{mode === 'login' ? 'Entrando...' : 'Cadastrando...'}</>
              ) : (
                <>{mode === 'login' ? 'Entrar' : 'Criar conta'}<ArrowRight className="h-4 w-4 ml-2" /></>
              )}
            </Button>
          </form>

          {onGoogleLogin && mode === 'login' && (
            <div className="mt-6">
              <Button variant="outline" className="w-full h-12 bg-background border-border/60 hover:bg-accent" type="button" onClick={onGoogleLogin}>
          ...
          <div className="text-center text-sm text-muted-foreground mt-8">
            {mode === 'login' ? (
              <>Não tem uma conta?{" "}
                <button onClick={() => { setMode('signup'); setLocalError(''); }} className="text-foreground font-medium hover:underline">Cadastre-se</button>
              </>
            ) : (
              <>Já tem uma conta?{" "}
                <button onClick={() => { setMode('login'); setLocalError(''); }} className="text-foreground font-medium hover:underline">Entrar</button>
              </>
            )}
          </div>
        </div>
...
export const Component = AnimatedCharactersLogin;
