...
export function PremiumPricingCard({ plan, annual, index }: PremiumPricingCardProps) {
  ...
  // Color schemes per card position
  const schemes = [
    { accent: 'purple', glow1: 'hsla(270, 80%, 50%, 1)', glow2: 'hsla(260, 90%, 75%, 1)', glow3: 'hsla(280, 70%, 60%, 1)', check: 'bg-purple-400', label: 'text-purple-400/80' },
    { accent: 'cyan', glow1: 'hsla(190, 100%, 40%, 1)', glow2: 'hsla(200, 100%, 85%, 1)', glow3: 'hsla(185, 100%, 55%, 1)', check: 'bg-cyan-300', label: 'text-cyan-400/80' },
    { accent: 'amber', glow1: 'hsla(35, 90%, 50%, 1)', glow2: 'hsla(40, 100%, 80%, 1)', glow3: 'hsla(30, 90%, 55%, 1)', check: 'bg-amber-400', label: 'text-amber-400/80' },
  ];
  ...
  return (
    <div
      className="premium-price-card group relative rounded-2xl p-6"
      style={{
    ...
    >
      ...
      {/* Popular badge */}
      {plan.highlight && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase tracking-wider bg-gradient-to-r from-cyan-400 to-blue-500 text-white px-4 py-1 rounded-full z-10 shadow-lg shadow-cyan-500/25">
          Mais popular
        </span>
      )}
