    ...
    <div className="w-full bg-[#0B0D15] pb-4" ref={containerRef}>
      ...
      <div ref={ref} className="relative max-w-5xl mx-auto pb-4">
          ...
          return (
            <div key={index} className="flex justify-start pt-4 md:pt-10 md:gap-6">
              ...
              <div className="relative pl-16 pr-4 md:pl-4 w-full">
                <h3 className="md:hidden block text-lg mb-2 text-left font-extrabold text-white/10 uppercase">
                  {item.title}
                </h3>
                <div className="space-y-1">
                  <h4 className="text-white text-sm md:text-base font-bold tracking-tight">{item.heading}</h4>
                  <ToolImage
                    src={item.images[0]}
                    alt={item.heading}
                    className="h-28 md:h-[170px] w-full"
                  />
                </div>
              </div>
            </div>
        ...
        <div
          ...
          className="absolute md:left-8 left-8 top-0 overflow-hidden w-[1.5px] bg-white/[0.03]"
        >
          <motion.div
            style={{ height: heightTransform, opacity: opacityTransform }}
            className="absolute inset-x-0 top-0 w-[1.5px] rounded-full overflow-hidden"
          >
