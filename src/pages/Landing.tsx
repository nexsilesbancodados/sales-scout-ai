...
const STEPS = [
  { icon: Search, step: '01', title: 'Capture leads', desc: 'A IA vasculha Google Maps, Instagram e Facebook para encontrar clientes ideais pro seu nicho.', color: '#7B2FF2', delay: 0, img: stepCaptureImg, backDetails: ['Google Maps & Places API', 'Instagram & Facebook Extractor', 'Filtros por nicho, cidade e avaliação', 'Deduplicação automática'] },
  { icon: MessageSquare, step: '02', title: 'Envie mensagens', desc: 'Mensagens personalizadas são enviadas no WhatsApp com anti-ban, spintax e delays humanizados.', color: '#F7941D', delay: 0.2, img: stepMessagesImg, backDetails: ['Spintax & variações automáticas', 'Anti-ban com delays humanizados', 'Warmup progressivo do chip', 'Personalização com nome e nicho'] },
  { icon: CalendarCheck, step: '03', title: 'Agende reuniões', desc: 'O agente SDR qualifica, tira objeções e agenda reuniões no seu calendário — 24/7.', color: '#00B4D8', delay: 0.4, img: stepMeetingsImg, backDetails: ['Agente SDR com IA DeepSeek', 'Qualificação BANT automática', 'Responde objeções em tempo real', 'Integração Google Meet'] },
];
          ...
          <div className="max-w-[1000px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            {STEPS.map((item, i) => (
              <motion.div
                ...
                style={{ perspective: '1000px', minHeight: 320 }}
              ...
              >
                ...
                >
                  ...
                  >
                    <div className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-700 pointer-events-none" style={{ background: `radial-gradient(circle at 50% 30%, ${item.color}18, transparent 70%)` }} />
                    ...
                    <div className="flex justify-center mb-4">
                      <div className="relative">
                        ...
                        <img src={item.img} alt={item.title} className="h-24 w-24 object-contain relative z-10 drop-shadow-[0_0_20px_rgba(123,47,242,0.2)]" loading="lazy" width={96} height={96} />
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-3 right-4 text-[10px] text-white/20 flex items-center gap-1">
                  ...
                  {/* BACK */}
                  <div
                    className="absolute inset-0 border rounded-2xl p-6 overflow-hidden flex flex-col justify-between"
                    style={{
                      backfaceVisibility: 'hidden',
                      transform: 'rotateY(180deg)',
                      borderColor: `${item.color}30`,
                      background: `linear-gradient(135deg, ${item.color}10, rgba(11,13,21,0.95))`,
                    }}
                  >
                    <div>
                      <div className="flex items-center gap-3 mb-5">
                        ...
                        <div>
                          <span className="text-[10px] uppercase tracking-[0.2em] font-semibold" style={{ color: item.color }}>{item.step}</span>
                          <h3 className="text-[15px] font-bold text-white">{item.title}</h3>
                        </div>
                      </div>

                      <ul className="space-y-3">
                        {item.backDetails.map((detail, j) => (
                          <li key={j} className="flex items-start gap-2.5 text-[13px] text-white/60">
                            <Check className="h-4 w-4 mt-0.5 shrink-0" style={{ color: item.color }} />
                            <span>{detail}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="text-[10px] text-white/20 text-center mt-4 flex items-center justify-center gap-1">
